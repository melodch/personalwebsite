const $element = $('.tattoo-animation');
...
function step(startTime) {
    if (active) {
        ...
        if (timeFromLastUpdate > timePerFrame) {
            ...
            $element.attr('src', `images/Logo/Tattoo-${frameNumber}.png`);
...
function reverseTattoo() {
    direction = (direction + 1) % 2;
}

function playPauseTattoo() {
    active = (active + 1) % 2;
}
...
$(document).ready(() => {
    for (var i = 1; i < totalFrames + 1; i++) {
        $('body').append(`<div id="preload-image-${i}" <img class="tattoo-animation" src=\`images/Logo/Neutuis-${frameNumber}.png\`></div>`);
    }
});
